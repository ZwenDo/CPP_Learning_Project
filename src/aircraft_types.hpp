...
struct AircraftType
{
    ...
    {
        assert(max_ground_speed > 0.0f && "max_ground_speed must be greater than 0.0f");
        assert(max_air_speed > 0.0f && "max_air_speed must be greater than 0.0f");
        assert(max_accel > 0.0f && "max_accel must be greater than 0.0f");
    }
};
